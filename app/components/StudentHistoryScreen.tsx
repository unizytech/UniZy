        ...
        <div className="flex items-center gap-3">
          {metadata && (
            <span className="text-xs text-gray-500">
              ...
              {metadata.counsellor_name && ` - ${metadata.counsellor_name}`}
            </span>
          )}
          <svg
...
export function StudentHistoryScreen() {
  ...
  // Load students when counsellor is selected
  useEffect(() => {
    const loadCounsellorStudents = async () => {
      if (!selectedCounsellorId) {
        setCounsellorStudents([]);
        return;
      }

      try {
        setIsLoadingCounsellorStudents(true);
        // Load all students for this counsellor (empty query returns all)
        const response = await searchStudents('', selectedCounsellorId, 1, 100, getAccessToken());
        setCounsellorStudents(response.students);
      } catch (err: any) {
        console.error('Failed to load counsellor students:', err);
        setCounsellorStudents([]);
      } finally {
        setIsLoadingCounsellorStudents(false);
      }
    };

    loadCounsellorStudents();
  }, [selectedCounsellorId]);

  // Debounced search (only when no counsellor selected)
  useEffect(() => {
    if (selectedCounsellorId) {
      // When counsellor is selected, use dropdown instead of search
      setSearchResults([]);
      return;
    }
  ...
  }, [searchQuery, selectedCounsellorId]);

  const handleSearch = async () => {
    try {
      setIsSearching(true);
      setError(null);
      const response = await searchStudents(searchQuery, selectedCounsellorId || undefined, 1, 20, getAccessToken());
  ...
  // Load prescreen data when student is selected
  const handleViewModeChange = (mode: 'prescreen' | 'consult_details') => {
    setViewMode(mode);
    if (mode === 'prescreen' && selectedStudent && !prescreenData && !isLoadingPrescreen) {
      // Use external student_id (not database UUID) for API calls
      loadPrescreenData(selectedStudent.student_id);
    }
  };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
              ...
              <div className="relative">
                <input
                  ...
                  placeholder={selectedCounsellorId ? "Type to filter students..." : "Search by name or student ID..."}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-gray-900 bg-white"
                />
                {(isSearching || isLoadingCounsellorStudents) && (
          ...
          {/* Right Panel - Student History Details */}
              ...
              <>
                ...
                <div className="flex items-center justify-between bg-white rounded-lg border border-gray-200 p-1">
                  ...
                  <div className="flex items-center gap-2">
                    {viewMode === 'prescreen' && (
                      <button
                        onClick={() => loadPrescreenData(selectedStudent.student_id)}
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-blue-700">
                                  ...
                                  {selectedConsultation?.counsellor_name && ` • ${selectedConsultation.counsellor_name}`}
                                </p>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">

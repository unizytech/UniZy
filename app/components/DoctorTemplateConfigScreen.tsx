...
/**
 * Doctor Template Configuration Screen - Dual View
 *
 * Shows doctors:
 ...
 *
 * Actions:
 * - Activate from Consultation Type → Creates new doctor-owned template (auto-activated)
...
import { getDoctor, setDoctorDefaultTemplate } from '@/services/doctorApi';
...
export default function DoctorTemplateConfigScreen() {
  ...
  // template-level column unrelated to the doctor's chosen default. We track
  // doctor.default_template_id directly and compare per-row.
  ...
  // Load dashboard data when doctor selected
  useEffect(() => {
    if (selectedDoctorId) {
      loadDashboard();
    } else {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      const [response, doctor] = await Promise.all([
        getDoctorDashboard(selectedDoctorId, token),
        getDoctor(selectedDoctorId, token).catch(() => null),
      ]);
      ...
      // Doctor record carries default_template_id (FK column on doctors table).
  ...
  const handleSetDefaultTemplate = async (templateId: string | null) => {
    ...
    try {
      const result = await setDoctorDefaultTemplate(
        selectedDoctorId,
        templateId,
        getAccessToken()
      );
  ...
  const handleActivateFromConsultationType = async () => {
    if (!selectedDoctorId || !selectedConsultationTypeId) return;
  ...
  const handleCloneTemplate = async () => {
    if (!selectedDoctorId || !selectedTemplateId) return;
  ...
  const handleCloneFromViewModal = async () => {
    if (!selectedDoctorId || !viewingTemplate) return;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {selectedDoctorId && !loading && !error && (
        <>
      ...
      {configuringTemplate && selectedDoctorId && (

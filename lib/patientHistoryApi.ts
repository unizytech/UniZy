/**
 * API Client for Patient History Retrieval
 *
 * Provides functions to interact with /api/v1/patients/* endpoints
 * for retrieving patient medical history data.
 *
...
export interface PatientSearchResult {
  ...
  consultation_count: number;
  last_visit_date: string | null;
  add_info?: Record<string, unknown> | null;  // Additional info (e.g., room/bed for NICU patients)
...
export interface PatientHistoryAllResponse {
  patient: PatientInfo;
...
export interface PrescreenResponse {
  ...
  top_interventions: InterventionSummary[];
  // Patient warning factors (CAUTION segment from last consultation) - can be object or string
...
export async function searchPatients(
  ...
  const response = await authGet(`/api/v1/patients/search?${params.toString()}`, accessToken ?? null);
...
export async function getConsultationHistory(
  ...
  const response = await authGet(
    `/api/v1/patients/${encodeURIComponent(patientId)}/consultations?${params.toString()}`,
    auth ?? null
  );
...
export async function getLastPrescription(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/last-prescription${params.toString() ? '?' + params.toString() : ''}`;
...
/**
 * Get last diagnosis for a patient
 */
export async function getLastDiagnosis(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/last-diagnosis${params.toString() ? '?' + params.toString() : ''}`;
  const response = await authGet(endpoint, auth ?? null);
...
/**
 * Get last investigation results for a patient
 */
export async function getLastInvestigationsResults(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/last-investigations-results${params.toString() ? '?' + params.toString() : ''}`;
  const response = await authGet(endpoint, auth ?? null);
...
/**
 * Get last investigations ordered for a patient
 */
export async function getLastInvestigationsOrdered(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/last-investigations-ordered${params.toString() ? '?' + params.toString() : ''}`;
  const response = await authGet(endpoint, auth ?? null);
...
/**
 * Get last case summary for a patient
 */
export async function getLastCaseSummary(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/last-case-summary${params.toString() ? '?' + params.toString() : ''}`;
  const response = await authGet(endpoint, auth ?? null);
...
/**
 * Get patient context (case summary + emotions + interventions)
 */
export async function getPatientContext(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/context${params.toString() ? '?' + params.toString() : ''}`;
  const response = await authGet(endpoint, auth ?? null);

  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.detail || `Failed to get patient context: ${response.statusText}`);
  }
...
/**
 * Get all patient history data in one request
 */
export async function getAllPatientHistory(
  patientId: string,
  doctorId?: string,
  auth?: string | AuthOptions | null
): Promise<PatientHistoryAllResponse> {
  const params = new URLSearchParams();
  if (doctorId) params.append('doctor_id', doctorId);

  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/history/all${params.toString() ? '?' + params.toString() : ''}`;
  const response = await authGet(endpoint, auth ?? null);

  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.detail || `Failed to get patient history: ${response.statusText}`);
  }
...
/**
 * Get multi-visit pattern analysis for a patient
...
export async function getMultiVisitPatterns(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/patterns/multi-visit?${params.toString()}`;
  const response = await authGet(endpoint, auth ?? null);
...
/**
 * Get emotion trend patterns for a patient
...
export async function getEmotionPatterns(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/patterns/emotions?${params.toString()}`;
  const response = await authGet(endpoint, auth ?? null);
...
/**
 * Get diagnosis pattern analysis for a patient
...
export async function getDiagnosisPatterns(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/patterns/diagnoses?${params.toString()}`;
  const response = await authGet(endpoint, auth ?? null);
...
/**
 * Get chief complaint patterns for a patient
...
export async function getComplaintPatterns(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/patterns/complaints?${params.toString()}`;
  const response = await authGet(endpoint, auth ?? null);
...
export async function getClinicalTimeline(
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/clinical-timeline?${params.toString()}`;
  const response = await authGet(endpoint, auth ?? null);
 ...
 * @param doctorId - Doctor UUID (required - prescreen data is doctor-specific)
 * @param auth - Bearer token or API key for authentication
 */
export async function getPatientPrescreen(
  patientId: string,
  doctorId: string,
  auth?: string | AuthOptions | null
  ...
  const endpoint = `/api/v1/patients/${encodeURIComponent(patientId)}/prescreen?${params.toString()}`;
  const response = await authGet(endpoint, auth ?? null);

  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    throw new Error(errorData.detail || `Failed to get patient prescreen: ${response.statusText}`);
  }

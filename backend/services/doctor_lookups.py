"""
Raster Doctor (seenBy) Lookup Tables

Maps doctor names to their Raster database IDs for the seenBy field.
...
# Default doctor ID when no match is found
DEFAULT_SEEN_BY_ID = 7  # Dr S Ramakrishnan


# ============================================================================
# DOCTOR DATABASE FROM RASTER
...
# ============================================================================
# KEYWORD / ALIAS MAP — common name variants to doctor IDs
# ============================================================================
...
def _normalize_doctor_name(name: str) -> str:
    ...
    for prefix in ["dr.", "dr ", "doctor ", "dr. "]:
        if normalized.startswith(prefix):
            normalized = normalized[len(prefix):].strip()
    return normalized


def _build_doctor_index() -> Dict[str, int]:
    """Build a normalized lookup index from doctor display names to IDs."""
    index = {}
    for doc_id, display_name in DOCTOR_DATABASE.items():
        normalized = _normalize_doctor_name(display_name)
...
DOCTOR_NAME_INDEX = _build_doctor_index()
...
def lookup_doctor_id(name: str) -> Optional[int]:
    """
    Look up a Raster doctor ID from a name string.
    ...
    Args:
        name: Doctor name extracted from audio

    Returns:
        Raster doctor ID if found, None otherwise
    """
    if not name:
        return None

    normalized = _normalize_doctor_name(name)
    if not normalized:
        return None
...
def resolve_seen_by_ids(seen_by_values: list) -> list:
    ...
    - String names are looked up in the doctor database
    - Unresolved strings are dropped

    Args:
        seen_by_values: List of doctor IDs (int) or names (str)

    Returns:
        List of resolved integer doctor IDs
    """
    ...
    for val in seen_by_values:
        if isinstance(val, int):
            ...
            if val_stripped.isdigit():
                ...
                doc_id = lookup_doctor_id(val_stripped)
...
def resolve_seen_by_single(value) -> int:
    ...
    Args:
        value: Doctor ID (int), name (str), or None

    Returns:
        Resolved integer doctor ID, or DEFAULT_SEEN_BY_ID if unresolved
    """

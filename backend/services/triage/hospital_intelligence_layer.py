"""
Hospital/Peer Intelligence Layer

Phase 2 of Triage Engine Multi-Layer system.
Provides peer intelligence and hospital-level pattern analysis:
- Same-specialty peer comparison
- Hospital investigation patterns
- Outlier detection
- Benchmark recommendations

This layer runs AFTER the doctor practice layer to add peer context.
"""
...
@dataclass
class HospitalPatterns:
    """Aggregated patterns for a hospital/specialty combination."""
    hospital_id: str
    specialty: str
    ...
    def has_sufficient_data(self) -> bool:
        ...
        return self.doctor_count >= 3 and self.total_feedback >= 20

    @classmethod
    def from_db_row(cls, row: Dict[str, Any]) -> Optional["HospitalPatterns"]:
        ...
        return cls(
            ...
            doctor_count=row.get("doctor_count") or 0,
...
class PeerComparison:
    ...
    def has_outliers(self) -> bool:
        """Check if doctor has any outlier metrics."""
        return len(self.outliers) > 0

    @classmethod
    def from_db_rows(cls, doctor_id: str, rows: List[Dict[str, Any]]) -> "PeerComparison":
        """Create from database rows."""
        comparison = cls(doctor_id=doctor_id)
...
class HospitalIntelligenceLayer:
    """
    Provides peer intelligence and hospital-level patterns.

    This layer:
    1. Fetches hospital patterns for doctor's specialty
    2. Compares doctor's patterns against peers
    ...
    async def get_hospital_patterns(
        self,
        hospital_id: str,
        specialty: str,
        supabase_client=None,
        force_recompute: bool = False
    ) -> Optional[HospitalPatterns]:
        """
        Get hospital patterns for a specialty.

        Args:
            hospital_id: UUID of the hospital
            specialty: Doctor specialty (e.g., "general_medicine")
            supabase_client: Supabase client for DB operations
            force_recompute: Force fresh computation

        Returns:
            HospitalPatterns object or None
        """
        client = supabase_client or self.supabase
        if not client or not hospital_id or not specialty:
            return None
        ...
        try:
            if force_recompute:
                ...
                result = client.table("hospital_specialty_patterns").select("*").eq(
                    "hospital_id", hospital_id
                ).eq("specialty", specialty).single().execute()

            if result.data:
                patterns = HospitalPatterns.from_db_row(result.data)
                logger.info(f"[HOSPITAL_LAYER] Loaded patterns for {hospital_id}/{specialty}: "
                           f"doctors={patterns.doctor_count}, extractions={patterns.total_extractions}")
                return patterns
            ...
            if result.data:
                return HospitalPatterns.from_db_row(result.data)

            return None

        except Exception as e:
            logger.warning(f"[HOSPITAL_LAYER] Failed to get hospital patterns: {e}")
            return None
    ...
    async def get_peer_comparison(
        ...
        """
        Get peer comparison metrics for a doctor.
        ...
        """
        client = supabase_client or self.supabase
        if not client or not doctor_id:
            return None
        ...
        try:
            ...
            if result.data:
                comparison = PeerComparison.from_db_rows(doctor_id, result.data)
                if comparison.has_outliers:
                    logger.info(f"[HOSPITAL_LAYER] Doctor {doctor_id} has {len(comparison.outliers)} outlier metrics")
                return comparison
    ...
    def detect_outliers(
        ...
        patterns: HospitalPatterns,
        doctor_style: Optional["DoctorPracticeStyle"] = None
    ) -> List[OutlierFlag]:
        """
        Detect outlier patterns in suggestions compared to hospital norms.

        Args:
            suggestions: Current triage suggestions
            patterns: Hospital patterns for comparison
            doctor_style: Optional doctor's practice style
    ...
    def enhance_with_peer_context(
        ...
        suggestions.gap_analysis["hospital_intelligence_applied"] = {
            "hospital_id": patterns.hospital_id,
            "specialty": patterns.specialty,
            "peer_doctor_count": patterns.doctor_count,
...
class HospitalPatternAggregator:
    """
    Background job service for aggregating hospital patterns.
    ...
    async def aggregate_all_patterns(
        ...
        """
        Run aggregation for all hospital/specialty combinations.
        ...
        try:
            result = client.rpc('compute_all_hospital_patterns').execute()
            ...
            for row in result.data or []:
                if row.get("success"):
                    success_count += 1
                    total_doctors += row.get("doctor_count", 0)
                else:
                    failure_count += 1
...
def get_hospital_intelligence_layer() -> HospitalIntelligenceLayer:
    """Get singleton HospitalIntelligenceLayer instance."""
    global _hospital_layer_instance
    if _hospital_layer_instance is None:
        _hospital_layer_instance = HospitalIntelligenceLayer()
    return _hospital_layer_instance


def get_hospital_aggregator() -> HospitalPatternAggregator:
    """Get singleton HospitalPatternAggregator instance."""
    global _aggregator_instance
    if _aggregator_instance is None:
        _aggregator_instance = HospitalPatternAggregator()

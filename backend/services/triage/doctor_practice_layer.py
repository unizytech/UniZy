"""
Doctor Practice Style Layer

Phase 1 of Triage Engine Multi-Layer system.
Enhances triage suggestions based on learned doctor practice patterns:
...
@dataclass
class DoctorPracticeStyle:
    """Aggregated practice style for a doctor."""
    doctor_id: str
    ...
    @classmethod
    def from_db_row(cls, row: Dict[str, Any]) -> "DoctorPracticeStyle":
...
class DoctorPracticeLayer:
    ...
    async def get_practice_style(
    ...
    ) -> Optional[DoctorPracticeStyle]:
        """
        Get practice style for a doctor (cached or freshly computed).
        ...
        Returns:
            DoctorPracticeStyle object or None if insufficient data
        """
        ...
        try:
            ...
            if result.data:
                style = DoctorPracticeStyle.from_db_row(result.data)
                logger.info(f"[PRACTICE_LAYER] Loaded practice style for doctor {doctor_id}: "
            ...
            logger.warning(f"[PRACTICE_LAYER] Failed to get practice style for {doctor_id}: {e}")
            return None
    ...
    async def compute_and_cache_style(
        ...
        Returns:
            Freshly computed DoctorPracticeStyle
        """
        client = supabase_client or self.supabase
        if not client or not doctor_id:
            return None
        ...
        try:
            ...
            if result.data:
                style = DoctorPracticeStyle.from_db_row(result.data)
                logger.info(f"[PRACTICE_LAYER] Recomputed practice style for doctor {doctor_id}")
                return style

            return None

        except Exception as e:
            logger.error(f"[PRACTICE_LAYER] Failed to compute practice style for {doctor_id}: {e}")
            return None

    def enhance_suggestions(
        self,
        suggestions: "TriageSuggestions",
        style: DoctorPracticeStyle
    ) -> "TriageSuggestions":
        """
        Enhance suggestions based on doctor's practice style.
        ...
        logger.info(f"[PRACTICE_LAYER] Enhancing suggestions for doctor {style.doctor_id} "
        ...
        for priority_name, suggestion_list in [
            ...
            for suggestion in suggestion_list:
                ...
                if suggestion.category == "investigation":
                    ...
                    if boost > 0:
                        suggestion.rationale += f" [Matches doctor's preferred investigation pattern]"
                        boosted_count += 1
        ...
        for suggestion in suggestions.nice_to_have:
            if suggestion.category == "investigation":
                ...
                if boost >= 5:  # Strong preference (5+ acceptances)
                    ...
                    promoted_count += 1
                    logger.debug(f"[PRACTICE_LAYER] Promoted (doctor preference): {suggestion.suggestion[:50]}...")
                else:
    ...
    def _get_preference_boost(
        ...
        """
        Get boost score based on doctor's investigation preferences.
    ...
    def _apply_intensity_adjustments(
        ...
        if style.practice_intensity == "conservative":
            # For conservative doctors, limit investigation suggestions
            inv_count = sum(
            ...
            if inv_count > 5:
                ...
                for s in suggestions.important_considerations:
                    if s.category == "investigation":
                        s.rationale += " [Doctor has conservative investigation pattern]"

        elif style.practice_intensity == "aggressive":
            # For aggressive doctors, could potentially add more suggestions
...
def get_practice_layer() -> DoctorPracticeLayer:
    """Get singleton DoctorPracticeLayer instance."""
    global _practice_layer_instance
    if _practice_layer_instance is None:
        _practice_layer_instance = DoctorPracticeLayer()

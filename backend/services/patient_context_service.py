"""
Patient Context Service for Prompt Injection

This service fetches relevant patient history data to inject into extraction prompts:
1. Past prescriptions - for medicine continuity when doctor says "continue previous medicines"
...
relevant patient history for more accurate extraction.
"""
...
def get_patient_context_for_extraction(
    patient_id: str,
    ...
    """
    Fetch relevant patient history for prompt injection.
    ...
    - New consultation mode (is_continuation=False, default): Fetch latest 1
      extraction. Prescriptions + cautions from all (current + linked doctors),
      summaries only from current doctor.

    Args:
        patient_id: Patient UUID string
        doctor_id: Optional doctor UUID string to filter consultations
    ...
    try:
        ...
        logger.warning(f"[PATIENT_CONTEXT] Invalid patient_id: {patient_id}")
        return result
    ...
    try:
        ...
        if is_continuation and parent_extraction_ids:
            ...
            parent_result = supabase.table("medical_extractions")\
            ...
            # Referral, Emergency Contact, Patient Info, Report Metadata from most recent parent
            ...
            if parent_patient_info:
                ...
                logger.info(f"[PATIENT_CONTEXT] Extracted patient information from parent {extraction_id}")
            ...
            result["has_context"] = bool(
                ...
                result.get("parent_nutritional_screening")
            )
        ...
        query = supabase.table("medical_extractions")\
            .select("id, doctor_id, created_at, original_extraction_json, edited_extraction_json, session_id")\
            .eq("patient_id", str(patient_uuid))\
            .order("created_at", desc=True)\
            .limit(fetch_limit)

        if doctor_id:
            # Expand to include linked doctors (via doctor_doctor_patients)
            from services.visit_detection_service import _get_linked_doctor_ids
            linked_ids = _get_linked_doctor_ids(doctor_id, patient_id=patient_id)
            if linked_ids:
                all_doctor_ids = [doctor_id] + linked_ids
                query = query.in_("doctor_id", all_doctor_ids)
            else:
        ...
        if not all_extractions:
            logger.debug(f"[PATIENT_CONTEXT] No past extractions found for patient {patient_id}")
            return result
        ...
        logger.info(
            f"[PATIENT_CONTEXT] Using latest 1 extraction for patient {patient_id} "
        ...
        for ext in extractions:
            ...
            # Prescription: from ALL (current + linked doctors)
            prescription = extract_and_normalize_prescription(ext_data, include_raw_text=True)
            if prescription:
                result["past_prescriptions"].append({
                    "extraction_id": extraction_id,
            ...
            # Summary: ONLY from current doctor's extractions (not linked doctors)
            # Prevents cross-doctor diagnosis leaking into new consultation context
            ...
            # Caution: from ALL (current + linked doctors) — safety first
            caution_segment = ext_segments.get("CAUTION")
            if caution_segment:
                all_cautions.append({
                    "extraction_id": extraction_id,
        ...
        if all_cautions:
            ...
            logger.info(
                f"[PATIENT_CONTEXT] Aggregated CAUTION from {len(all_cautions)} extractions for patient {patient_id}"
            )
        ...
        logger.error(f"[PATIENT_CONTEXT] Error fetching patient context: {e}", exc_info=True)
        return result
...
def format_patient_context_for_prompt(context: Dict[str, Any], is_continuation: bool = False) -> str:
    """
    Format patient context data into a string for prompt injection.
    ...
    Args:
        context: Patient context dict from get_patient_context_for_extraction()
        is_continuation: Whether this is a continuation of a prior visit (default: False)
    ...
    if is_continuation:
        # CONTINUATION MODE header — Gemini is the SOLE merge engine
        sections.append("""
==============================================================================
SAME-VISIT PRIOR RECORDINGS (CONTINUATION MODE — YOU ARE THE SOLE MERGE ENGINE)
==============================================================================
The following data is from the MOST RECENT prior recording in this SAME patient visit.
It already contains cumulative context from earlier recordings in the chain.

YOU ARE THE PRIMARY MERGE ENGINE. A lightweight validation check runs after you to catch
cross-segment inconsistencies, but YOUR output is the foundation. Produce the most
accurate, COMPLETE merged record as if both recordings were a single consultation.
Do NOT drop data just because the doctor didn't repeat it in this recording.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
MERGE PRINCIPLES (CRITICAL — READ CAREFULLY):
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

1. MEDICATION TRACKING (most important for prescriptions):
   - If doctor STOPS a medicine (e.g., "stop Dolo", "discontinue Paracetamol"):
     → DO NOT include that medicine in the output. It is removed.
   - PARTIAL STOPS: Doctor may stop ONLY SOME medicines. "Stop A and B" means ONLY A and B
     are removed — all other prior medicines MUST be carried forward unchanged.
   - If doctor CHANGES dosage/frequency (e.g., "increase Metformin to 1000mg"):
     → Include the medicine with the UPDATED dosage/frequency only.
   - If doctor says "continue previous medicines" or does not mention a medicine at all:
     → Carry forward ALL prior medicines unchanged.
   - If doctor ADDS a new medicine: Include it alongside carried-forward medicines.
   - NEVER produce duplicate medicine entries.

2. LATEST-WINS FOR CURRENT STATE (vitals, examination, current medications):
   - If the current transcript provides a value → use it (replaces prior).
   - If the current transcript does NOT mention it → carry forward prior value unchanged.
   - Produce clean, unified values — NO "|" separators, NO concatenation of old and new.

3. APPEND-MODE FOR HISTORY (past medical history, investigation results, HPI):
   - Combine prior and current information chronologically.
   - Show disease progression: what was known before + what's new now.
   - Produce a coherent narrative, NOT "prior text | current text".

4. NARRATIVE SYNTHESIS (for text fields like summary, treatmentPlan notes):
   - Produce a single coherent narrative covering both recordings.
   - Show progression where relevant (e.g., "Initially presented with X, now also Y").
   - NEVER concatenate with "|" separators — synthesize into natural text.

5. CHIEF COMPLAINTS:
   - ALWAYS carry forward ALL prior chief complaints.
   - Add any NEW complaints from the current transcript.
   - Only REMOVE a complaint if doctor explicitly says it is resolved.

6. DIAGNOSIS:
   - ALWAYS carry forward ALL prior diagnoses (with ICD codes).
   - Add any NEW diagnoses from the current transcript.
   - Only REMOVE a diagnosis if the doctor explicitly contradicts or resolves it.

7. INVESTIGATIONS:
   - Carry forward all prior investigations.
   - If doctor cancels an investigation → remove it from the list.
   - If doctor adds new investigations → add them to the list.
   - If results are available for a prior investigation → update that specific investigation
     with the results AND include the date/time context if mentioned in transcript
     (e.g., "CBC done yesterday shows normal" → update CBC with results and date reference).
   - PARTIAL RESULTS: Only update investigations whose results are actually discussed.
     Other investigations remain in their current state (e.g., "ordered").
     Do NOT assume all investigations got results just because some did.

8. CONFLICT RESOLUTION:
   - The CURRENT transcript is ALWAYS the source of truth when there is a conflict.
   - If the CURRENT transcript states a DIFFERENT diagnosis, use the CURRENT one.
   - If the CURRENT transcript changes the treatment plan, use the CURRENT plan.
   - Prior context provides continuity for data NOT mentioned in the current transcript.

9. Prior cautions/allergies MUST always be checked for safety (warnings segment).

10. DIAGNOSIS REFINEMENT:
    - When a diagnosis is refined (e.g., "Suspected Pneumonia" → "Bacterial Pneumonia"),
      output ONLY the refined version. Do NOT output both the suspected and confirmed forms.
    - When a complaint is resolved, REMOVE it from chiefComplaints. Do not keep resolved
      complaints alongside active ones.

11. ALLERGY-PRESCRIPTION SAFETY:
    - If a new allergy is discovered that conflicts with a prior medication, you MUST
      REMOVE that medication from the prescription. Patient safety overrides carry-forward rules.
    - Example: Prior had Amoxicillin, now patient found allergic to Amoxicillin →
      DO NOT include Amoxicillin in prescription output.

12. CROSS-SEGMENT CONSISTENCY CHECK:
    Before finalizing your output, verify these cross-segment consistency rules:

    a) INVESTIGATION ↔ RESULTS: Investigation results discussed in transcript → update
       that specific investigation (do not still show "ordered" when results are available).
       Include result date context if mentioned. Only update investigations whose results
       were actually discussed — leave others unchanged.

    b) VITALS ↔ RESOLUTION: If "fever resolved" or similar → do NOT carry forward the
       abnormal vital sign value. Either omit the temperature or note "afebrile" / "resolved".

    c) REFERRAL ↔ FULFILLMENT: If this consultation fulfills a prior referral (e.g., prior
       said "refer to cardiologist" and this IS the cardiologist visit) → remove or mark
       the referral as completed.

    d) ALLERGY ↔ PRESCRIPTION: No prescribed drug should conflict with known allergies
       in the allergy segment. If conflict found, REMOVE the drug.
==============================================================================""")
    else:
        # NEW CONSULTATION MODE header (original)
        sections.append("""
==============================================================================
PAST PATIENT RECORDS (FOR REFERENCE TO GENERATE CAUTION & WARNINGS)
==============================================================================
The following are the patient's PAST prescriptions and consultation summaries.
...
3. Provide relevant history context ONLY when the doctor explicitly references it
...
==============================================================================""")
    ...
    if context.get("caution_aggregated"):
        ...
        if caution_text:
            ...
            sections.append(f"""
**PATIENT SAFETY ALERTS - AGGREGATED FROM PAST CONSULTATIONS (CRITICAL):**
{source_info}
...
⚠️ CRITICAL - NEVER OUTPUT VAGUE PHRASES unless the PATIENT's Past prescription doesn't have actual medicine names:
...
✅ REQUIRED ACTIONS:
1. **When doctor says "continue previous medicines", "same medicines", "continue the same", etc.**:
...
2. **When doctor mentions a specific medicine from past prescription**:
   - Use the EXACT name and dosage from the past prescriptions above
   - Match case and spelling exactly

3. **When doctor modifies a previous medicine** (e.g., "increase Paracetamol dosage"):
...
""")
                ...
                sections.append(f"""
**PATIENT'S PAST CONSULTATION SUMMARIES:**
{summaries_text}
...
- Only REMOVE a complaint if the doctor explicitly says it is resolved in the current recording.
...
- Add any NEW diagnoses from the current transcript.
- Only REMOVE a diagnosis if the doctor explicitly contradicts or resolves it in the current recording.
...
INSTRUCTIONS:
- Only carry forward investigations that were CONFIRMED/ORDERED by the doctor.
...
- If doctor CANCELS an investigation (e.g., "cancel the MRI", "no need for X-ray") → REMOVE it.
- Deduplicate by test name — if same investigation appears in both, use the current version.
- Do not include Investigation RESULTS discussed by the doctor (e.g., "blood sugar came back 180")
...
""")
    ...
    # Format prior patient information section (continuation auto-merge only)
    if is_continuation and context.get("parent_patient_info"):
        ...
        sections.append(f"""
**PRIOR PATIENT INFORMATION (CARRY FORWARD — LATEST-WINS):**
{pi_text}

INSTRUCTIONS:
- If current transcript has patient info (age, gender, ward, bed) → use current.
...
INSTRUCTIONS:
- If current transcript has metadata (doctor name, location, specialty) → use current.
- Otherwise carry forward prior values.
""")
...
- If current transcript reveals a NEW condition (e.g., "patient now reports diabetes"), update that specific entry to "Yes" with duration.
...
- Only change has_allergy from "Yes" to "No Known Allergy" if doctor explicitly corrects it.
""")

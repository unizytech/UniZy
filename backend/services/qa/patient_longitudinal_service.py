"""
Patient Longitudinal Service

Provides longitudinal data aggregation and comparison for patient visits:
...
class PatientLongitudinalService:
    """
    Service for longitudinal patient data analysis.

    Usage:
        service = PatientLongitudinalService()
    ...
    async def get_changes_since_visit(
        ...
        try:
            ...
            baseline_result = supabase.table("medical_extractions")\
                .select("*")\
            ...
            if current_extraction_id:
                current_result = supabase.table("medical_extractions")\
                    .select("*")\
                    .eq("id", str(current_extraction_id))\
                    .single()\
                    .execute()
            else:
                # Get most recent extraction for patient
                # Note: medical_extractions doesn't have hospital_id, filter through doctors join
                current_result = supabase.table("medical_extractions")\
                    .select("*, doctors!inner(hospital_id)")\
    ...
    async def get_longitudinal_summary(
        ...
        try:
            ...
            query = supabase.table("medical_extractions")\
                .select("*, doctors!inner(hospital_id, full_name), consultation_types(type_name)")\
                .eq("patient_id", str(patient_id))\
                .eq("doctors.hospital_id", str(hospital_id))\
                .order("created_at", desc=True)\
                .limit(num_visits)
            ...
            if not visits:
                return {"error": "No visits found for patient"}
            ...
            for visit in reversed(visits):  # Chronological order
                ...
                medication_history.append({
                    ...
                    "doctor_name": visit.get("doctors", {}).get("full_name") if visit.get("doctors") else None,
                ...
                "visits": [
                    {
                        ...
                        "doctor_name": v.get("doctors", {}).get("full_name") if v.get("doctors") else None
                    }
                    for v in visits
                ]
            }
    ...
    async def get_single_visit_data(
        ...
        Args:
            ...
            hospital_id: Hospital context for validation

        Returns:
            Dict with extraction_id, created_at, doctor_name, consultation_type_name,
        ...
        try:
            ...
            result = supabase.table("medical_extractions")\
                .select("*, doctors!inner(full_name, hospital_id), consultation_types(type_name)")\
            ...
            doctor_info = row.get("doctors") or {}
    ...
    async def synthesize_single_visit_narrative(
        ...
        try:
            ...
            doctor_name = visit_data.get("doctor_name", "Unknown")
            ...
            system_prompt = """You are a medical assistant helping answer questions about a specific patient visit.
    ...
    async def synthesize_multi_visit_narrative(
        ...
        try:
            ...
            for med_entry in medication_history:
                ...
                doctor = med_entry.get("doctor_name", "Unknown doctor")
            ...
            system_prompt = """You are a medical assistant helping summarize patient history across multiple visits.
...
Patient Visit Summary ({total_visits} visits):
...
Generate a clear narrative response answering the user's question about their history across these visits."""
            ...
            response = client.models.generate_content(
                ...
                contents=[
                    {"role": "user", "parts": [{"text": system_prompt}]},
                    {"role": "model", "parts": [{"text": "I'll provide clear summaries of patient visit history."}]},
                    {"role": "user", "parts": [{"text": user_prompt}]}
                ],
                config={
                    "temperature": 0.3,
    ...
    async def synthesize_narrative(
        ...
        try:
            ...
            system_prompt = """You are a medical assistant helping summarize changes between patient visits.
Generate a clear, clinically relevant narrative that directly answers the user's question.
...
Generate a clear narrative response answering the user's question about what changed."""
            ...
            response = client.models.generate_content(
                ...
                contents=[
                    {"role": "user", "parts": [{"text": system_prompt}]},
                    {"role": "model", "parts": [{"text": "I'll provide clear summaries of patient visit changes."}]},
                    {"role": "user", "parts": [{"text": user_prompt}]}
                ],
                config={
                    "temperature": 0.3,
...
# Singleton instance
patient_longitudinal_service = PatientLongitudinalService()

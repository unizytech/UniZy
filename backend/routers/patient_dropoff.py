"""
Patient Dropoff Risk API Router

Provides endpoints to retrieve patient dropoff risk (retention risk) assessments.

Patient dropoff risk identifies 5 churn indicators:
...
from services.supabase_service import (
    ...
    get_patient_dropoff_history,
    get_patient_latest_dropoff_risk,
...
class DropoffRiskResponse(BaseModel):
    """Response model for patient dropoff risk assessment."""
    id: str
    extraction_id: str
...
async def get_dropoff_for_extraction(extraction_id: str):
    """
    Get patient dropoff risk assessment for a specific extraction.
...
async def get_patient_dropoff_history_endpoint(
    patient_id: str,
    limit: int = Query(default=10, le=50, ge=1)
) -> List[DropoffHistoryItem]:
    """
    Get dropoff risk assessment history for a patient.
    ...
    history = get_patient_dropoff_history(patient_id, limit)
...
@router.get("/patient/{patient_id}/latest", response_model=Optional[DropoffRiskResponse])
async def get_patient_latest_dropoff(patient_id: str):
    """
    Get the most recent dropoff risk assessment for a patient.
    ...
    risk = get_patient_latest_dropoff_risk(patient_id)
...
async def get_high_risk_patients_endpoint(
    doctor_id: Optional[str] = None,
    limit: int = Query(default=50, le=100, ge=1)
) -> List[HighRiskPatient]:
    """
    ...
    Returns:
        List of high-risk patients ordered by probability descending
    """
    if doctor_id:
        try:
            uuid.UUID(doctor_id)
    ...
    patients = get_high_risk_patients(doctor_id, limit)

    result = []
    for item in patients:
        extraction_data = item.get("medical_extractions", {}) or {}
        result.append(HighRiskPatient(
            id=item["id"],
            extraction_id=item["extraction_id"],
...
async def get_dropoff_statistics_endpoint(
    ...
    stats = get_dropoff_risk_statistics(doctor_id, days)

"""
Patient History API Router

Provides endpoints to retrieve patient medical history including:
1. Last prescription
...
6. Patient context (case summary + emotional analysis + recommended interventions)

All endpoints support filtering by doctor_id and hospital_id.

Security:
- When AUTH_ENABLED=true, all endpoints require authentication
- EHR clients: Auto-create patients if they don't exist
- Doctor access: Validated against client's allowed_doctor_ids
...
from services.raster_api_service import fetch_all_neopaed_patients
from services.audit_service import audit_service
from services.history_extraction_utils import (
    # Nurse extraction filtering (with legacy aliases)
    filter_nurse_extractions,
    is_nurse_extraction,
...
if AUTH_ENABLED:
    from dependencies.auth import get_current_client, PatientAccessChecker, EHRPatientAccessChecker, EHRDoctorAccessChecker
    from services.audit_service import audit_service
...
class PatientInfo(BaseModel):
    ...
    id: str
    patient_id: str  # External identifier
...
class PatientContextResponse(BaseModel):
    """Complete patient context for informed consultations"""
    patient: PatientInfo
...
class PatientSearchResult(BaseModel):
    ...
    consultation_count: int = 0
    last_visit_date: Optional[str] = None
    add_info: Optional[Dict[str, Any]] = None  # Additional info (e.g., room/bed for NICU patients)
    hospital_id: Optional[str] = None
    hospital_name: Optional[str] = None


class PatientSearchResponse(BaseModel):
    """Response for patient search"""
...
class PatientCreateRequest(BaseModel):
    ...
    doctor_ids: Optional[List[str]] = None  # Array of doctor UUIDs
...
class PatientCreateResponse(BaseModel):
    ...
    created: bool = False  # True if new patient, False if already existed
    message: str
...
class ConsultationHistoryResponse(BaseModel):
    """Response for patient consultation history"""
    patient: PatientInfo
...
# ============================================================================
# Hospital Context Helper
# ============================================================================

def _get_hospital_id_from_context(request: Request, doctor_id: Optional[str] = None) -> Optional[str]:
    """
    Extract hospital_id from auth context or derive from doctor_id.

    Priority:
    1. Auth client context (EHR clients have hospital_id)
    2. Derive from doctor_id via cached lookup
    ...
    if AUTH_ENABLED:
        try:
            client = get_current_client(request)
            if hasattr(client, 'hospital_id') and client.hospital_id:
                return str(client.hospital_id)
        except Exception:
            pass

    # Derive from doctor_id
    if doctor_id:
        from services.supabase_service import get_doctor_hospital_id_cached
        return get_doctor_hospital_id_cached(doctor_id)
...
def get_patient_info(patient_uuid: uuid.UUID) -> Optional[Dict[str, Any]]:
    """Get patient basic info by UUID"""
    result = supabase.table("patients")\
        .select("id, patient_id, full_name, date_of_birth, gender, preferred_language")\
...
def resolve_patient_id(patient_id: str, hospital_id: Optional[str] = None) -> Optional[uuid.UUID]:
    """
    Resolve external patient_id to internal database UUID.

    The patient_id parameter can be either:
    1. An external identifier (patient_id column) - PRIMARY lookup method
    2. A database UUID (id column) - fallback if external ID not found

    External systems typically only have patient_id (varchar), so we prioritize
    that lookup first for better performance and compatibility.

    Args:
        patient_id: External patient identifier (preferred) or database UUID
        hospital_id: Hospital UUID for scoped lookup (optional)

    Returns:
        Database UUID or None if patient not found
    """
    # PRIORITY 1: Try to find by external patient_id first
    # This is the primary lookup method since external systems use patient_id
    ...
    try:
        potential_uuid = uuid.UUID(patient_id)
        result = supabase.table("patients")\
            .select("id")\
...
def get_patient_info_by_external_id(patient_id: str, hospital_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """
    Get patient info by external patient_id (or database UUID).

    Args:
        patient_id: External patient identifier or database UUID
        hospital_id: Hospital UUID for scoped lookup (optional)

    Returns:
        Patient info dict or None if not found
    """
    patient_uuid = resolve_patient_id(patient_id, hospital_id=hospital_id)
    if not patient_uuid:
        return None
    return get_patient_info(patient_uuid)


def resolve_patient_id_or_404(patient_id: str, hospital_id: Optional[str] = None) -> uuid.UUID:
    """
    Resolve external patient_id to internal database UUID, raising HTTPException if not found.
...
def resolve_patient_with_info_or_404(patient_id: str, hospital_id: Optional[str] = None) -> Tuple[uuid.UUID, Dict[str, Any]]:
    """
    Resolve patient_id and get patient info in a single query.
    Combines resolve_patient_id_or_404() + get_patient_info() to save 1 DB round-trip.
    """
    # Try by external patient_id first
    query = supabase.table("patients")\
...
def get_doctor_name(doctor_id: str) -> Optional[str]:
...
def batch_get_doctor_names(doctor_ids: List[str]) -> Dict[str, str]:
    """
    Batch fetch doctor names by IDs.
    Returns dict mapping doctor_id -> full_name.
    ...
    unique_ids = list(set(id for id in doctor_ids if id))
    if not unique_ids:
        return {}
...
def build_extraction_metadata(extraction: Dict[str, Any]) -> ExtractionMetadata:
    ...
    doctor_name = None
    if extraction.get("doctor_id"):
        doctor_name = get_doctor_name(extraction["doctor_id"])
...
def _get_hospital_view_doctor_ids(
    current_doctor_id: str,
    patient_uuid: uuid.UUID,
) -> List[str]:
    """
    Resolve the OTHER active doctor IDs at the same hospital as `current_doctor_id`,
    after verifying the patient also belongs to that hospital.

    Used by patient-history endpoints when `hospital_view=true` is requested:
    callers want to see records authored by other staff (doctors and
    nurses-on-behalf-of-doctors, since every extraction carries a doctor_id) within
    the same hospital — excluding the requesting doctor's own records.

    Raises:
        HTTPException 400 if doctor or patient lacks hospital_id, or 403 if their
        hospitals don't match.
    """
    from services.supabase_service import get_doctor_hospital_id_cached

    doctor_hospital_id = get_doctor_hospital_id_cached(current_doctor_id)
    if not doctor_hospital_id:
        raise HTTPException(
            status_code=400,
            detail="Cannot use hospital_view: requesting doctor has no hospital_id."
        )
    ...
    patient_row = supabase.table("patients")\
        ...
        .limit(1)\
        .execute()
    patient_hospital_id = (patient_row.data or [{}])[0].get("hospital_id")
    if not patient_hospital_id:
        raise HTTPException(
            status_code=400,
            detail="Cannot use hospital_view: patient has no hospital_id."
        )
    if str(patient_hospital_id) != str(doctor_hospital_id):
        raise HTTPException(
            status_code=403,
            detail="Patient does not belong to the requesting doctor's hospital."
        )

    others = supabase.table("doctors")\
        .select("id")\
        .eq("hospital_id", str(doctor_hospital_id))\
        .neq("id", current_doctor_id)\
        .eq("is_active", True)\
        .execute()
    return [row["id"] for row in (others.data or [])]


def _resolve_hospital_view_filter(
    hospital_view: bool,
    doctor_id: Optional[str],
    patient_uuid: uuid.UUID,
) -> Optional[List[str]]:
    """
    When hospital_view=True, return the list of OTHER-doctor IDs at the same
    hospital (after verifying patient hospital match). Returns None when the
    flag is off so the caller can fall back to the existing single-doctor path.
    Raises 400 if hospital_view=True but no doctor_id provided.
    """
    if not hospital_view:
        return None
    if not doctor_id:
        raise HTTPException(
            status_code=400,
            detail="hospital_view=true requires the doctor_id query parameter."
        )
    return _get_hospital_view_doctor_ids(doctor_id, patient_uuid)


def get_latest_extraction_for_patient(
    patient_uuid: uuid.UUID,
    doctor_id: Optional[str] = None,
    hospital_id: Optional[str] = None,
    doctor_ids: Optional[List[str]] = None,
    include_nurse: bool = False,
) -> Optional[Dict[str, Any]]:
    """
    Get the most recent NON-PRESCREEN extraction for a patient.

    PRESCREEN extractions are filtered out as they typically don't have meaningful
    clinical data (prescriptions, diagnoses, etc.).

    When `doctor_ids` is provided (hospital_view mode), it takes precedence over
    `doctor_id` and matches `doctor_id IN (...)`. When `include_nurse=True`,
    nurse-initiated extractions are not filtered out.
    """
    # Get recent extractions with recording_sessions join for PRESCREEN filtering
    query = supabase.table("medical_extractions")\
        .select("*, recording_sessions(template_code, nurse_id)")\
    ...
    if doctor_ids is not None:
        if not doctor_ids:
            return None
        query = query.in_("doctor_id", doctor_ids)
    elif doctor_id:
        query = query.eq("doctor_id", doctor_id)

    # Hospital filtering would require joining with doctors table
    ...
    # Find first non-nurse extraction (nurse_id check + PRESCREEN legacy fallback)
    for extraction in result.data:
        if not include_nurse and is_nurse_extraction(extraction):
            continue
        return extraction
...
def build_emotion_pattern_summary(
    ...
    `doctor_ids` (hospital_view mode) overrides `doctor_id` and filters via IN().
    `include_nurse=True` skips the nurse-extraction filter.
    """
    ...
    if pre_fetched_extractions is not None:
        ...
        emotion_query = supabase.table("medical_extractions")\
            .select("id, created_at, recording_sessions(template_code, nurse_id)")\
            .eq("patient_id", str(patient_uuid))\
        ...
        if doctor_ids is not None:
            ...
            emotion_query = emotion_query.in_("doctor_id", doctor_ids)
        elif doctor_id:
            emotion_query = emotion_query.eq("doctor_id", doctor_id)
        ...
        if include_nurse:
            # Hospital-view mode wants nurse-authored data included as-is
            filtered_extractions = (emotion_result.data or [])[:num_visits]
        else:
            # Filter out nurse extractions (nurse_id + PRESCREEN legacy fallback)
            filtered_extractions = filter_nurse_extractions(emotion_result.data or [], max_results=num_visits)
...
def get_latest_extraction_with_segment(
    ...
    When `doctor_ids` is provided (hospital_view mode), it takes precedence over
    `doctor_id` and matches `doctor_id IN (...)`. When `include_nurse=True`, nurse
    extractions are not skipped.
    """
    # Get recent extractions for patient with recording_sessions join for PRESCREEN filtering
    query = supabase.table("medical_extractions")\
        .select("*, recording_sessions(template_code, nurse_id)")\
        .eq("patient_id", str(patient_uuid))\
        .order("created_at", desc=True)
    ...
    # Find first non-nurse extraction with requested segment
    for extraction in result.data:
        if not include_nurse and is_nurse_extraction(extraction):
            continue
...
def get_top_interventions(
    ...
    Args:
        ...
        doctor_ids: Optional list — when provided (hospital_view mode), filters by
            doctor_id IN (doctor_ids) and overrides `doctor_id`.
        include_nurse: When True, nurse-initiated extractions are not skipped.
    ...
    if pre_fetched_extraction:
        ...
        # Fallback: fetch from DB (used when called outside prescreen endpoint)
        query = supabase.table("medical_extractions")\
            .select("id, recording_sessions(template_code, nurse_id)")\
            .eq("patient_id", str(patient_uuid))\
            .order("created_at", desc=True)
        ...
        if result_data.data:
            for ext in result_data.data:
                if include_nurse or not is_nurse_extraction(ext):
                    extraction_id = ext["id"]
                    break

        if not extraction_id:
            extraction = get_latest_extraction_for_patient(
                patient_uuid, doctor_id,
                doctor_ids=doctor_ids, include_nurse=include_nurse,
            )
            if not extraction:
                return []
            extraction_id = extraction["id"]
    ...
    interventions_result = supabase.table("patient_interventions")\
...
def get_caution_and_summary_from_last_extraction(
    ...
    """
    Get CAUTION and SUMMARY segments from patient's last NON-PRESCREEN extraction.
...
def get_latest_nurse_extraction(
    patient_uuid: uuid.UUID,
    doctor_id: str
) -> Optional[Dict[str, Any]]:
    """
    Get the latest nurse-initiated extraction for a patient.

    Nurse extractions are identified by:
    1. recording_sessions.nurse_id IS NOT NULL (primary)
    2. template_code containing 'PRESCREEN' (legacy fallback for records with NULL nurse_id)

    Args:
        patient_uuid: Patient UUID
        doctor_id: Doctor ID (required for lookup)

    Returns:
        Extraction record or None if no nurse extraction found
    """
    # Fetch recent extractions and filter in Python (PostgREST can't filter on FK nurse_id reliably)
    query = supabase.table("medical_extractions")\
        .select("*, recording_sessions!inner(template_code, template_name, nurse_id)")\
        .eq("patient_id", str(patient_uuid))\
    ...
    # Primary: find extraction with nurse_id set
...
# Legacy alias
get_latest_prescreen_extraction = get_latest_nurse_extraction
...
def build_clinical_timeline_data(
    ...
    """
    Build clinical timeline data for a patient.
    ...
    Args:
        ...
        doctor_ids: Optional list — when provided (hospital_view mode), filters by
            doctor_id IN (doctor_ids) and overrides `doctor_id`.
        include_nurse: When True, nurse-initiated extractions are not filtered out.
    ...
    try:
        ...
        all_extractions_result = supabase.table("medical_extractions")\
            .select("original_extraction_json, edited_extraction_json, created_at, recording_sessions(template_code, nurse_id)")\
            .eq("patient_id", str(patient_uuid))\
            .order("created_at", desc=True)\
            .limit(30)\
            .execute()
        ...
        for i, ext in enumerate(extractions):
            ...
            doctor_name = get_doctor_name(ext["doctor_id"]) if ext.get("doctor_id") else None
...
def get_last_prescription_for_prescreen(
    ...
    """
    Get the last prescription for a patient from a NON-PRESCREEN extraction.
...
async def create_patient(
    ...
    **Fields:**
    - patient_id: Required - External identifier (UHID/MRN)
    - full_name: Patient's full name
    - date_of_birth: Birth date in ISO format (YYYY-MM-DD)
    - gender: M/F
    - ip_id: Inpatient visit ID
    - op_id: Outpatient visit ID
    - doctor_ids: Array of doctor UUIDs to link patient to
    - add_info: Additional metadata as JSON object

    **Behavior:**
    - If patient_id already exists, returns existing patient with created=False
    - If patient_id is new, creates patient and returns with created=True
    """
    try:
        # Derive hospital_id from auth context or first doctor_id
        create_hospital_id = _get_hospital_id_from_context(request, body.doctor_ids[0] if body.doctor_ids else None)

        # Check if patient already exists (scoped by hospital)
        check_query = supabase.table("patients").select("*").eq("patient_id", body.patient_id)
        if create_hospital_id:
            check_query = check_query.eq("hospital_id", create_hospital_id)
        ...
        if existing.data:
            ...
            return PatientCreateResponse(
                ...
                created=False,
                message=f"Patient with ID {body.patient_id} already exists"
            )
        ...
        if create_hospital_id:
            patient_data["hospital_id"] = create_hospital_id
        if body.full_name:
            patient_data["full_name"] = body.full_name
        if body.date_of_birth:
            patient_data["date_of_birth"] = body.date_of_birth
        if body.gender:
            patient_data["gender"] = body.gender
        if body.ip_id:
            patient_data["ip_id"] = body.ip_id
        if body.op_id:
            patient_data["op_id"] = body.op_id
        if body.doctor_ids:
            patient_data["doctor_ids"] = body.doctor_ids
        if body.add_info:
            patient_data["add_info"] = body.add_info

        # Create new patient
        result = supabase.table("patients").insert(patient_data).execute()

        if result.data:
            logger.info(f"[PATIENT_CREATE] Created patient {body.patient_id}")
            return PatientCreateResponse(
                success=True,
                patient=result.data[0],
                created=True,
                message=f"Patient {body.patient_id} created successfully"
            )
        else:
            return PatientCreateResponse(
                success=False,
        ...
        logger.error(f"[PATIENT_CREATE] Error creating patient {body.patient_id}: {e}")
        raise HTTPException(status_code=500, detail="Failed to create patient")


# ============================================================================
# API Endpoints - Patient Search
# ============================================================================

# Doctor SKS ID - triggers neopaed patient initialization
DOCTOR_SKS_ID = "397f4efe-fd79-41d1-9b0b-d3cc884cda62"

@router.get("/search", response_model=PatientSearchResponse)
async def search_patients(
    request: Request,
    query: Optional[str] = Query(None, description="Search by name or patient ID"),
    doctor_id: str = Query(..., description="Doctor ID (required for EHR access)"),
    ...
    Returns patients with consultation count and last visit date.
    Optimized to minimize database queries.

    **Note:** doctor_id is required for EHR access control.
    **Special:** For Doctor SKS, automatically syncs patients from Neopaed hospital system.
    """
    try:
        offset = (page - 1) * page_size

        # For Doctor SKS, initialize/sync patients from Neopaed hospital system (both NICU + OP)
        if doctor_id == DOCTOR_SKS_ID:
            logger.info("[PATIENT_SEARCH] Doctor SKS - initializing Neopaed patients (NICU + OP)")
            try:
                init_result = await fetch_all_neopaed_patients()
        ...
        # If doctor_id is provided, get patients directly from extractions (more efficient)
        if doctor_id:
            # Get all extractions for this doctor with patient info in one query
            ext_result = supabase.table("medical_extractions")\
                .select("patient_id, created_at, patients!inner(id, patient_id, full_name, date_of_birth, gender, hospital_id)")\
                .eq("doctor_id", doctor_id)\
                .order("created_at", desc=True)\
                .execute()
            ...
            for ext in (ext_result.data or []):
                ...
                patient_stats[pid]["count"] += 1

            # Include patients who have this doctor in their doctor_ids array
            # Uses PostgreSQL array contains operator
            doctor_linked_result = supabase.table("patients")\
                .select("id, patient_id, full_name, date_of_birth, gender, add_info, doctor_ids, hospital_id, updated_at")\
                .contains("doctor_ids", [doctor_id])\
                .execute()

            for p in (doctor_linked_result.data or []):
                pid = p["id"]
                if pid not in patient_stats:
                    # Add patient without any consultations (linked via doctor_ids)
                    patient_stats[pid] = {
                        "patient": {
                            "id": p["id"],
            ...
            # For Doctor SKS, also include Neopaed patients who may not have extractions yet
            if doctor_id == DOCTOR_SKS_ID:
                neopaed_result = supabase.table("patients")\
                    .select("id, patient_id, full_name, date_of_birth, gender, add_info, hospital_id, updated_at")\
                    .not_.is_("add_info", "null")\
                    .execute()

                for p in (neopaed_result.data or []):
                    # Check if this is a neopaed-sourced patient (NICU or OP)
                    add_info = p.get("add_info") or {}
                    source = add_info.get("source", "")
                    if source in ("neopaed_api", "neopaed_op_api"):
                        pid = p["id"]
                        if pid not in patient_stats:
                            # Add patient without any consultations
                            patient_stats[pid] = {
                                "patient": {
                                    "id": p["id"],
                                    "patient_id": p["patient_id"],
            ...
            if query:
                ...
                patient_stats = {
                    pid: stats for pid, stats in patient_stats.items()
                    if (stats["patient"].get("full_name") or "").lower().find(query_lower) >= 0 or
                       (stats["patient"].get("patient_id") or "").lower().find(query_lower) >= 0
                }

            # Sort by last visit (most recent first), patients without visits at the end
            sorted_patients = sorted(
                patient_stats.values(),
                key=lambda x: (x["last_visit"] is not None, x["last_visit"] or ""),
                reverse=True
            )

            total_count = len(sorted_patients)
            paginated = sorted_patients[offset:offset + page_size]

            # Resolve hospital names for paginated patients
            hospital_ids = list(set(
                p["patient"].get("hospital_id")
                for p in paginated
                if p["patient"].get("hospital_id")
            ))
            hospital_names: Dict[str, str] = {}
            if hospital_ids:
                try:
                    h_result = supabase.table("hospitals")\
                        .select("id, hospital_name")\
                        .in_("id", hospital_ids)\
                        .execute()
                    hospital_names = {h["id"]: h["hospital_name"] for h in (h_result.data or [])}
                except Exception as e:
                    logger.warning(f"[PATIENT_SEARCH] Failed to resolve hospital names: {e}")
            ...
            results = [
                PatientSearchResult(
                    ...
                    hospital_id=p["patient"].get("hospital_id"),
                    hospital_name=hospital_names.get(p["patient"].get("hospital_id") or "", None)
                )
                for p in paginated
            ]

        else:
            # No doctor filter - search all patients
            base_query = supabase.table("patients")\
                .select("id, patient_id, full_name, date_of_birth, gender, hospital_id")

            if query:
                base_query = base_query.or_(
                    f"patient_id.ilike.%{query}%,full_name.ilike.%{query}%"
                )
            ...
            paginated_patients = all_patients[offset:offset + page_size]

            if paginated_patients:
                # Get consultation stats for paginated patients in one query
                patient_ids = [p["id"] for p in paginated_patients]
                ext_result = supabase.table("medical_extractions")\
                ...
                for ext in (ext_result.data or []):
                    pid = ext["patient_id"]
                    if pid not in patient_stats:
                        patient_stats[pid] = {"count": 0, "last_visit": ext["created_at"]}
                    patient_stats[pid]["count"] += 1

                # Resolve hospital names
                h_ids = list(set(p.get("hospital_id") for p in paginated_patients if p.get("hospital_id")))
                ...
                if h_ids:
                    try:
                        h_result = supabase.table("hospitals")\
                            ...
                            .in_("id", h_ids)\
                            .execute()
                        h_names = {h["id"]: h["hospital_name"] for h in (h_result.data or [])}
                    except Exception as e:
                        logger.warning(f"[PATIENT_SEARCH] Failed to resolve hospital names: {e}")
                ...
                results = [
                    PatientSearchResult(
                        ...
                        date_of_birth=p.get("date_of_birth"),
                        gender=p.get("gender"),
                        consultation_count=patient_stats.get(p["id"], {}).get("count", 0),
                        last_visit_date=patient_stats.get(p["id"], {}).get("last_visit"),
                        hospital_id=p.get("hospital_id"),
                        hospital_name=h_names.get(p.get("hospital_id") or "", None)
                    )
...
# ============================================================================
# API Endpoints - Patient History
# ============================================================================

@router.get("/{patient_id}/consultations", response_model=ConsultationHistoryResponse)
async def get_consultation_history(
    request: Request,
    patient_id: str,
    doctor_id: Optional[str] = Query(None, description="Filter by doctor ID"),
    hospital_view: bool = Query(False, description="When true, return data authored by OTHER doctors/nurses at the same hospital (excludes the requesting doctor's own records). Requires doctor_id."),
    page: int = Query(1, ge=1),
    page_size: int = Query(20, ge=1, le=100),
    ...
    """
    try:
        # Fix 1: Single query for patient resolution + info (saves 1-2 round-trips)
        hospital_id = _get_hospital_id_from_context(request, doctor_id)
        patient_uuid, patient_info = resolve_patient_with_info_or_404(patient_id, hospital_id=hospital_id)
        view_doctor_ids = _resolve_hospital_view_filter(hospital_view, doctor_id, patient_uuid)
        ...
        def _apply_doctor_filter(q):
            if view_doctor_ids is not None:
                if not view_doctor_ids:
                    return None  # signal: empty result
                return q.in_("doctor_id", view_doctor_ids)
            if doctor_id:
                return q.eq("doctor_id", doctor_id)
            return q
        ...
        def _fetch_extractions():
            q = supabase.table("medical_extractions")\
                .select(
                    "id, session_id, consultation_type_id, doctor_id, created_at, "
        ...
        doctor_ids = list(set(ext.get("doctor_id") for ext in extractions if ext.get("doctor_id")))
        ...
        doctor_names_map, ct_names_map, segments_map, submission_ids_map = await asyncio.gather(
            loop.run_in_executor(_prescreen_executor, batch_get_doctor_names, doctor_ids),
        ...
        for ext in extractions:
            ...
            consultations.append(ConsultationHistoryItem(
                ...
                doctor_id=ext_doctor_id,
                doctor_name=doctor_names_map.get(ext_doctor_id) if ext_doctor_id else None,
        ...
        # HIPAA Audit: log patient consultation history access
...
@router.get("/{patient_id}/consultations/latest", response_model=ConsultationHistoryResponse)
async def get_latest_consultations(
    ...
    """
    Get latest consultations for a patient, de-duplicated by continuation chain.
    ...
    parent_extraction_ids array for the same patient.
    """
    try:
        hospital_id = _get_hospital_id_from_context(request, doctor_id)
        patient_uuid, patient_info = resolve_patient_with_info_or_404(patient_id, hospital_id=hospital_id)
        view_doctor_ids = _resolve_hospital_view_filter(hospital_view, doctor_id, patient_uuid)
        patient_uuid_str = str(patient_uuid)
        ...
        def _fetch_all_parent_ids():
            result = supabase.table("medical_extractions")\
                ...
                .eq("patient_id", patient_uuid_str)\
        ...
        def _fetch_extractions():
            ...
            if view_doctor_ids is not None:
                if not view_doctor_ids:
                    return type("R", (), {"data": []})()
                query = query.in_("doctor_id", view_doctor_ids)
            elif doctor_id:
                query = query.eq("doctor_id", doctor_id)
        ...
        # Step 4: Enrich with doctor names, consultation type names, preview segments
        doctor_ids_list = list(set(ext.get("doctor_id") for ext in page_extractions if ext.get("doctor_id")))
        ...
        doctor_names_map, ct_names_map, segments_map, submission_ids_map = await asyncio.gather(
            loop.run_in_executor(_prescreen_executor, batch_get_doctor_names, doctor_ids_list),
            loop.run_in_executor(_prescreen_executor, batch_get_consultation_type_names, ct_ids),
            loop.run_in_executor(_prescreen_executor, _fetch_preview_segments),
            loop.run_in_executor(_prescreen_executor, _fetch_submission_ids),
        )
...
@router.get("/{patient_id}/last-prescription", response_model=LastPrescriptionResponse)
async def get_last_prescription(
    ...
    """
    Get the last prescription for a patient.

    Searches through recent extractions to find the most recent prescription data.
    """
    try:
        hospital_id = _get_hospital_id_from_context(request, doctor_id)
        patient_uuid = resolve_patient_id_or_404(patient_id, hospital_id=hospital_id)
        view_doctor_ids = _resolve_hospital_view_filter(hospital_view, doctor_id, patient_uuid)

        # Get patient info
        patient_info = get_patient_info(patient_uuid)
        if not patient_info:
            raise HTTPException(status_code=404, detail="Patient not found")
        ...
        extraction = get_latest_extraction_with_segment(
            patient_uuid, prescription_keys, doctor_id,
            doctor_ids=view_doctor_ids, include_nurse=hospital_view,
        )
        ...
        if client_ctx:
            try:
                asyncio.create_task(audit_service.log_phi_access(
                    client_context=client_ctx, request=request, response_status=200,
                    response_time_ms=0, resource_type="patient", action="read",
                    patient_id=patient_id, phi_fields=["prescription"],
                ))
            except Exception:
                pass
...
@router.get("/{patient_id}/last-diagnosis", response_model=LastDiagnosisResponse)
async def get_last_diagnosis(
    ...
    """
    Get the last diagnosis for a patient.
    """
    try:
        hospital_id = _get_hospital_id_from_context(request, doctor_id)
        patient_uuid = resolve_patient_id_or_404(patient_id, hospital_id=hospital_id)
        ...
        extraction = get_latest_extraction_with_segment(
            patient_uuid, diagnosis_keys, doctor_id,
            doctor_ids=view_doctor_ids, include_nurse=hospital_view,
        )
...
@router.get("/{patient_id}/last-investigations-results", response_model=LastInvestigationsResponse)
async def get_last_investigations_results(
    ...
    """
    Get the last investigation results for a patient.
    ...
    try:
        ...
        extraction = get_latest_extraction_with_segment(
            patient_uuid, result_keys, doctor_id,
            doctor_ids=view_doctor_ids, include_nurse=hospital_view,
        )
...
@router.get("/{patient_id}/last-investigations-ordered", response_model=LastInvestigationsResponse)
async def get_last_investigations_ordered(
    ...
    """
    Get the last investigations ordered for a patient.
    ...
    try:
        ...
        extraction = get_latest_extraction_with_segment(
            patient_uuid, ordered_keys, doctor_id,
            doctor_ids=view_doctor_ids, include_nurse=hospital_view,
        )
...
@router.get("/{patient_id}/last-case-summary", response_model=LastCaseSummaryResponse)
async def get_last_case_summary(
    ...
    """
    Get the last case summary for a patient.
        ...
        if client_ctx:
            try:
                asyncio.create_task(audit_service.log_phi_access(
                    client_context=client_ctx, request=request, response_status=200,
                    response_time_ms=0, resource_type="patient", action="read",
                    patient_id=patient_id, phi_fields=["case_summary"],
                ))
            except Exception:
                pass
...
async def get_patient_context(
    ...
    """
    Get complete patient context for informed consultations.
    ...
    try:
        ...
        ext_query = supabase.table("medical_extractions")\
        ...
        if view_doctor_ids is not None:
            if not view_doctor_ids:
                ...
                ext_query = ext_query.in_("doctor_id", view_doctor_ids)
                ext_result_data = (ext_query.execute().data or [])
        elif doctor_id:
            ext_query = ext_query.eq("doctor_id", doctor_id)
            ext_result_data = (ext_query.execute().data or [])
        else:
            ext_result_data = (ext_query.execute().data or [])
        ...
        latest_extraction = get_latest_extraction_for_patient(
            patient_uuid, doctor_id,
            doctor_ids=view_doctor_ids, include_nurse=hospital_view,
        )
        ...
        raise HTTPException(status_code=500, detail="Failed to get patient context")
...
class PatientHistoryAllResponse(BaseModel):
    """Combined response with all patient history data"""
    patient: PatientInfo
...
class PrescreenResponse(BaseModel):
    """
    Prescreen information for a patient before consultation.
    ...
    4. Patient warning factors (CAUTION segment - allergies, contraindications)
    ...
    # 4. Patient warning factors (CAUTION segment from last consultation)
...
@router.get("/{patient_id}/history/all", response_model=PatientHistoryAllResponse)
async def get_all_patient_history(
    ...
    """
    Get all patient history data in a single request.
    ...
    Use this endpoint to get all data needed for the patient history screen.
    """
    try:
        hospital_id = _get_hospital_id_from_context(request, doctor_id)
        patient_uuid = resolve_patient_id_or_404(patient_id, hospital_id=hospital_id)
        ...
        data_query = supabase.table("medical_extractions")\
            .select("*, recording_sessions(template_code, nurse_id)")\
            .eq("patient_id", str(patient_uuid))\
            .order("created_at", desc=True)
        ...
        if view_doctor_ids is not None:
            if not view_doctor_ids:
                ...
                data_query = data_query.in_("doctor_id", view_doctor_ids)
                data_rows = data_query.limit(40).execute().data or []
        else:
            if doctor_id:
                data_query = data_query.eq("doctor_id", doctor_id)
            data_rows = data_query.limit(40).execute().data or []

        # Filter out PRESCREEN extractions (unless hospital_view wants nurse data shown)
        if hospital_view:
        ...
        if view_doctor_ids is not None:
            if not view_doctor_ids:
                ...
                emotion_query = emotion_query.in_("doctor_id", view_doctor_ids).limit(1)
        ...
        emotion_pattern_summary = build_emotion_pattern_summary(
            patient_uuid, doctor_id, num_visits=2,
            doctor_ids=view_doctor_ids, include_nurse=hospital_view,
        )
        ...
        return PatientHistoryAllResponse(
            patient=patient,
        ...
        logger.error(f"Error getting all patient history: {e}")
        raise HTTPException(status_code=500, detail="Failed to get all patient history")
...
@router.get("/{patient_id}/patterns/multi-visit", response_model=MultiVisitSummaryResponse)
async def get_multi_visit_summary(
    ...
    Use this to understand patient history trends before a consultation.
    """
    try:
        hospital_id = _get_hospital_id_from_context(request, doctor_id)
        patient_uuid = resolve_patient_id_or_404(patient_id, hospital_id=hospital_id)
        ...
        if view_doctor_ids is not None:
            if not view_doctor_ids:
                ...
                query = query.in_("doctor_id", view_doctor_ids).limit(num_visits * 2)
...
@router.get("/{patient_id}/patterns/emotions", response_model=EmotionPatternResponse)
async def get_emotion_patterns(
    ...
    Use this to understand patient's emotional patterns and identify concerning trends.
    """
    try:
        hospital_id = _get_hospital_id_from_context(request, doctor_id)
        patient_uuid = resolve_patient_id_or_404(patient_id, hospital_id=hospital_id)
        ...
        if view_doctor_ids is not None:
            if not view_doctor_ids:
                ...
                query = query.in_("doctor_id", view_doctor_ids).limit(num_visits)
        ...
        count_query = supabase.table("medical_extractions")\
            .select("id", count="exact")\
            .eq("patient_id", str(patient_uuid))
        if view_doctor_ids is not None:
            if not view_doctor_ids:
                ...
                count_query = count_query.in_("doctor_id", view_doctor_ids)
...
@router.get("/{patient_id}/patterns/diagnoses", response_model=DiagnosisPatternResponse)
...
@router.get("/{patient_id}/patterns/complaints", response_model=ChiefComplaintPatternResponse)
...
async def get_clinical_timeline(
    ...
    - "First Time" = never seen in patient history
    ...
    try:
        ...
        for ext in (all_extractions_result.data or []):
            # Skip nurse extractions for historical diagnosis lookup as well
            if is_nurse_extraction(ext):
                continue
...
@router.get("/{patient_id}/prescreen", response_model=PrescreenResponse)
async def get_patient_prescreen(
    request: Request,
    patient_id: str,
    ...
    hospital_view: bool = Query(False, description="When true, return data authored by OTHER doctors/nurses at the same hospital (excludes the requesting doctor's own records)."),
    _auth = Depends(verify_patient_access)
):
    """
    Get prescreen information for a patient before consultation.

    The patient_id parameter accepts either:
    - External patient identifier (e.g., "PAT-001", MRN, etc.)
    - Database UUID
    ...
    This endpoint requires doctor_id as prescreen data is doctor-specific.
    """
    try:
        # Phase 1: Sequential - resolve patient (required before everything else)
        effective_hospital_id = hospital_id or _get_hospital_id_from_context(request, doctor_id)
        patient_uuid = resolve_patient_id_or_404(patient_id, hospital_id=effective_hospital_id)
        view_doctor_ids = _resolve_hospital_view_filter(hospital_view, doctor_id, patient_uuid)
        ...
        def _fetch_prescreen():
            # Prescreen by definition is nurse-authored under a specific doctor.
            # In hospital_view mode, no single "owning" doctor → skip.
            if hospital_view:
                return None
            return get_latest_prescreen_extraction(patient_uuid, doctor_id)
        ...
        # (in hospital_view mode we keep nurse-authored rows so users can see them)
        if hospital_view:
        ...
        def _build_interventions():
            return get_top_interventions(
                patient_uuid, doctor_id, limit=3,
        ...
        def _build_timeline():
            return build_clinical_timeline_data(
                patient_uuid, doctor_id, num_visits=5,
                pre_fetched_extractions=non_prescreen_extractions,
                doctor_ids=view_doctor_ids, include_nurse=hospital_view,
            )
        ...
        logger.error(f"Error getting patient prescreen: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to get patient prescreen")


# ============================================================================
# Neopaed Patient Initialization
# ============================================================================
...
class NeopaedInitResponse(BaseModel):
    """Response for Neopaed patient initialization (NICU + OP combined)"""
    success: bool
    created: int = 0
    updated: int = 0
...
async def initialize_neopaed_patients():
    """
    Initialize patients from Neopaed hospital system (NICU + OP combined).

    Fetches patients from both hospital APIs and syncs to database:
    - NICU inpatients (get-nicu-inpatient-list)
    - OP babies (today-op-baby-list)

    For each patient:
    - Creates new patients if they don't exist (using uhid as patient_id)
    - Updates existing patients' add_info field with latest data
    - Stores visitNumber/visitId, roomNo, bedNo, gestation, etc. in add_info JSONB

    Returns:
        NeopaedInitResponse with counts of created/updated patients (total + per source)
    """
    try:
        result = await fetch_all_neopaed_patients()
        return NeopaedInitResponse(**result)
    except Exception as e:
        logger.error(f"Error initializing Neopaed patients: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to initialize Neopaed patients")

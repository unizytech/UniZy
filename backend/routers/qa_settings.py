...
async def get_current_model(
    hospital_id: Optional[UUID] = Query(None, description="Hospital ID (optional)"),
    client: ClientContext = Depends(get_current_client)
):
    """
    Get the currently active embedding model.

    If hospital_id is provided, returns the model configured for that hospital.
    Otherwise, returns the global default model.
    """
    from services.qa.embedding_service import embedding_service

    # Use hospital from client context if not provided
    effective_hospital_id = hospital_id or client.hospital_id

    model_config = await embedding_service.get_active_model(effective_hospital_id)
...
async def set_embedding_model(
    ...
    hospital_id: UUID = Query(..., description="Hospital ID to configure"),
    client: ClientContext = Depends(require_admin)
):
    """
    Set the active embedding model for a hospital.
    ...
    **Available models:**
    ...
    supabase.table("qa_engine_settings")\
        .upsert(settings_data, on_conflict="hospital_id")\
        .execute()

    logger.info(f"Set embedding model {request.model_code} for hospital {hospital_id}")
...
async def trigger_reembedding(
    ...
    """
    Trigger re-embedding of all extractions for a hospital.
...
@router.get("/hospital/{hospital_id}", response_model=QAEngineSettings)
async def get_hospital_settings(
    hospital_id: UUID,
    client: ClientContext = Depends(get_current_client)
):
    """
    Get Q&A Engine settings for a hospital.
    """
    from services.supabase_service import supabase
    ...
    if not result.data:
        ...
        return QAEngineSettings(
            ...
            allow_cross_doctor_search=False,
...
async def update_hospital_settings(
    ...
    """
    Update Q&A Engine settings for a hospital.
    ...
    **Admin only.**
    ...
    if request.allow_cross_doctor_search is not None:
        update_data["allow_cross_doctor_search"] = request.allow_cross_doctor_search
    ...
    supabase.table("qa_engine_settings")\
        .upsert(update_data, on_conflict="hospital_id")\
        .execute()
